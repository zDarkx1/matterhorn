...
<head>
    ...
    <title>{{ $product->name }} | Matterhorn Adventure Rental</title>
...
<body class="bg-white text-brand-black antialiased font-sans" data-product-stock="{{ $product->stock_available }}">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <nav class="mb-6" aria-label="Breadcrumb">
            <ol class="flex items-center gap-2 text-sm text-gray-500">
                ...
                <li><a href="/#katalog" class="hover:text-brand-orange transition">{{ $product->category }}</a></li>
                <li><span class="text-gray-300">/</span></li>
                <li class="text-brand-black font-medium truncate max-w-[200px]" aria-current="page">{{ $product->name }}</li>
            </ol>
        </nav>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
            ...
            <div>
                <div class="bg-gray-100 aspect-square flex items-center justify-center overflow-hidden group relative">
                    @if($product->image)
                        <img id="mainImage" src="{{ asset($product->image) }}" alt="{{ $product->name }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
                    @else
            ...
            <div class="flex flex-col">
                ...
                <div class="flex items-center gap-2 mb-3">
                    <span class="border border-gray-300 text-gray-600 text-xs font-bold uppercase tracking-wider px-3 py-1">{{ ucfirst($product->gender ?? 'Unisex') }}</span>
                    <span class="border border-gray-300 text-gray-600 text-xs font-bold uppercase tracking-wider px-3 py-1">{{ $product->category }}</span>
                </div>

                <!-- Name -->
                <h1 class="font-display font-bold text-2xl sm:text-3xl uppercase text-brand-black leading-tight mb-4">{{ $product->name }}</h1>

                <!-- Price -->
                <div class="mb-4">
                    <p class="text-brand-orange font-bold text-2xl sm:text-3xl font-display">
                        Rp {{ number_format($product->price_24h, 0, ',', '.') }}
                ...
                <div class="flex items-center gap-2 mb-6">
                    ...
                    @if($product->stock_available > 0)
                        <span class="flex items-center gap-1 text-green-600 font-medium text-sm">
                            ...
                            {{ $product->stock_available }} tersedia
                        </span>
                    @else
                    ...
                    <div class="flex flex-wrap gap-2" id="sizeSelector" role="radiogroup" aria-label="Pilih ukuran">
                        @foreach($product->sizes as $size)
                            <button
                                ...
                                class="size-btn border-2 border-gray-300 text-sm font-medium px-5 py-2.5 hover:border-brand-orange hover:text-brand-orange transition focus:outline-none focus:ring-2 focus:ring-brand-orange focus:ring-offset-1 {{ $size->stock <= 0 ? 'opacity-40 cursor-not-allowed line-through' : 'cursor-pointer' }}"
                                role="radio"
                                aria-checked="false"
                                aria-label="Ukuran {{ $size->size }}{{ $size->stock <= 0 ? ' - Habis' : '' }}"
                ...
                <div class="mb-6">
                    ...
                    <div class="flex items-center border border-gray-300 w-fit">
                        ...
                        <input id="qtyInput" type="number" value="1" min="1" max="{{ $product->stock_available }}" class="w-14 text-center text-sm font-medium outline-none border-x border-gray-300 py-2.5" aria-label="Jumlah item">
        ...
        <section class="mt-12 border-t border-gray-200 pt-10">
            ...
            @if($product->description)
                <div class="text-gray-600 text-sm leading-relaxed max-w-3xl whitespace-pre-line">{{ $product->description }}</div>
            @else
                ...
                <table class="w-full text-sm" role="presentation">
                    <tbody>
                        <tr class="border-b border-gray-100">
                            ...
                            <td class="py-3 text-brand-black font-medium text-right">{{ $product->category }}</td>
                        </tr>
                        <tr class="border-b border-gray-100">
                            <td class="py-3 text-gray-500 font-medium">Gender</td>
                            <td class="py-3 text-brand-black font-medium text-right">{{ ucfirst($product->gender ?? 'Unisex') }}</td>
                        </tr>
                        <tr class="border-b border-gray-100">
                            <td class="py-3 text-gray-500 font-medium">Stok Total</td>
                            <td class="py-3 text-brand-black font-medium text-right">{{ $product->stock_total }} unit</td>
                        </tr>
                        <tr>
                            <td class="py-3 text-gray-500 font-medium">ID Produk</td>
                            <td class="py-3 text-brand-black font-medium text-right">#{{ $product->id }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
